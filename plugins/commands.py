    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        btn = [
            [
                InlineKeyboardButton(
                    "🤖 Join Updates Channel", url='https://t.me/+C3LqoARFYiQwNjhl'
                )
            ]
        ]
        ...
        if message.command[1] != "subscribe":
            try:
                ...
                btn.append([InlineKeyboardButton(" 🔄 Already Joined", callback_data=f"{pre}#{file_id}")])
            except (IndexError, ValueError):
                btn.append([InlineKeyboardButton(" 🔄 Already Joined / Try Again", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}")])
